/**
 * One-time migration of speakasap-portal/seven/exercises/*.py into DrillItem.
 ...
 * pure `parseLegacyExerciseFile` / `parseTemplate` / `hashItem` /
 * `lessonOrderFromClassName` / `courseKeyFor` functions. This lets the
 * importer be exercised on a host with no reachable database (this
 * ecosystem requires k8s-mediated Postgres access). Apply mode (real
 ...
 *
 * Usage:
 *   npx ts-node scripts/import-seven-bank.ts <path-to-portal> --dry-run
 *   npx ts-node scripts/import-seven-bank.ts <path-to-portal>          # apply (writes DB)
 */
...
interface FileReport {
  ...
  /** Class name did not match `Lesson<N>Ex<M>` — cannot be placed in a lesson, useless for the baseline. */
  itemsSkippedNoLesson: number;
  ...
  /** Count of items in this file whose class matched a lesson and so carry a non-null lessonOrder. */
  itemsWithLessonOrder: number;
}
...
/** Pure parse pass: filesystem + parser + template + mapping only. No Prisma, no network. */
  ...
  for (const { path, dir } of entries) {
    ...
    if (!report.empty) {
      ...
      for (const cls of parseLegacyExerciseFile(source, path)) {
        ...
        if (lessonOrder === null) {
          report.itemsSkippedNoLesson += cls.items.length;
          continue;
        }
...
function totalsOf(reports: FileReport[]) {
  return reports.reduce(
    (a, r) => ({
      ...
      skippedNoLesson: a.skippedNoLesson + r.itemsSkippedNoLesson,
      ...
      itemsWithLessonOrder: a.itemsWithLessonOrder + r.itemsWithLessonOrder,
    }),
    {
      files: 0,
      emptyFiles: 0,
...
/** Per-(languageCode) breakdown of items carrying a non-null lessonOrder — Task A.6 needs this to be > 0 per language. */
function lessonOrderByLanguage(reports: FileReport[]): Record<string, number> {
  const out: Record<string, number> = {};
  for (const r of reports) {
    out[r.languageCode] = (out[r.languageCode] ?? 0) + r.itemsWithLessonOrder;
  }
  return out;
}
  ...
  try {
    for (const { path, dir } of entries) {
      ...
      for (const cls of parseLegacyExerciseFile(source, path)) {
        ...
        const sevenLesson = await prisma.sevenLesson.findFirst({
          where: { order: lessonOrder, course: { languageId: language.id, materialLanguage: ml } },
        });
        if (!sevenLesson) report.topicsUnmatched.push(slug);
        ...
        const topic = await prisma.drillTopic.upsert({
          ...
          create: {
            ...
            materialLanguage: ml,
            title: sevenLesson ? sevenLesson.title : slug.replace(/-/g, ' '),
          },
          update: {},
        });
...
async function main() {
  ...
  if (!portalRoot) throw new Error('usage: import-seven-bank.ts <path-to-portal> [--dry-run]');

  const base = join(portalRoot, 'seven/exercises');
  ...
  if (dryRun) {
      ...
      JSON.stringify(
        {
          ...
          // topics unmatched" when the truth is "not measured". Topic/SevenLesson matching
          // requires a DB lookup, which this mode never performs.
          topicsUnmatched: null,
          topicsUnmatchedNote:
            'not measured in --dry-run (requires SevenLesson DB lookups, which this mode never ' +
            'performs). Unlike the grammar bank, SevenLesson is populated (136 rows) and lessonOrder ' +
            'gives a mechanical match via (course.languageId, course.materialLanguage, order) — so ' +
            'apply mode is expected to resolve most of these.',
          lessonOrderByLanguage: lessonOrderByLanguage(reports),
          reports,
        },
        null,
        2,
    ...
    console.log(
      'NOTE: dry-run performed zero database queries (Prisma was never imported). ' +
        'Topic/SevenLesson matching and true (DB-level) duplicate detection require apply mode, ' +
  ...
  const totals = reports.reduce(
    ...
    { inserted: 0, duplicateInDb: 0, skippedNoBlanks: 0, skippedNoLesson: 0, topicsUnmatched: 0, languagesMissing: 0 },
  );
