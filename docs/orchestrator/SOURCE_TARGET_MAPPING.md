...
| `education_lessonrecord` | `LessonRecord` | Preserve legacy `uuid` as target primary key and preserve legacy `lesson_id` as unique target lesson relation. | Requires target lesson UUID. Target schema/migration exists locally and still needs remote validation/deploy. | Missing lesson blocks load; duplicate legacy record for lesson must be listed. | Write-gated script reports states, missing target lessons, duplicate records, target conflicts, and media/key reconciliation issues. |
| `education_lessonrecordpart` or JSONB `parts` | `LessonRecordPart` plus preserved record `parts` JSON | Preserve source part UUID when a referenced part row exists; preserve part object key as private metadata. | Requires source record membership and object key. | Missing record or multi-record part reference blocks apply; orphan part rows are reported but not imported into FK-owned target table. | Write-gated script reports orphan parts, missing part rows, storage-key classes, duplicate/multi-reference issues, and target part UUID conflicts. |
