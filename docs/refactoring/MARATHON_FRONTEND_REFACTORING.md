...
- [x] **7.2** Verify redirect chain: speakasap.com/marathon, speakasap.com/marathon/german/ → marathon.alfares.cz with correct path (done 2026-04-09 via curl: `/marathon/` → `https://marathon.alfares.cz`, `/marathon/german/` → `https://marathon.alfares.cz/german/`).
...
- [x] **7.4** Monitor logs baseline check completed (2026-04-09): marathon logger now supports blue/green-safe fallback host resolution (`logging-microservice`, `logging-microservice-backend-green`, `logging-microservice-backend-blue`) so deployment color switches do not break centralized logging; `GET https://logging.alfares.cz/api/logs/services` lists `marathon.alfares.cz`.
