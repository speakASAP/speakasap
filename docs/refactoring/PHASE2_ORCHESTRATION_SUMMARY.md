...
8. **Scheduled follow-up (non-blocking):** **F2-HTTP-JWT** — run `PHASE2_VALIDATION_REPORT.md` §3 matrix (C2–C8, A2–A8) when certification + assessment containers are deployed and routed via standard service deploy scripts; update report rows DEF→PASS; complete `PHASE2_CUTOVER_CHECKLIST.md` § Deploy/smoke. **2026-04-12:** routing probe recorded in report **§3.1**; operator handoff **`docs/superpowers/cursor-tasks/task-01-f2-http-jwt-smoke.md`** (matrix still **DEF** until HTTPS upstream + stable assessment + JWT evidence).
