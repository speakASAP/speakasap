...
- `NOTIFICATION_SERVICE_URL` and `INTERNAL_API_TOKEN` must be present in
  education-service's environment. `NOTIFICATION_SERVICE_URL` exists in
  ...
  `dispatch/email` is JWT-guarded, so `INTERNAL_API_TOKEN` must be a token that
  guard accepts — verify this at deploy time; it was not exercised here.
