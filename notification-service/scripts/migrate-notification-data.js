...
const SPEAKASAP_ROOT = (0, node_path_1.join)(process.cwd(), '..');
...
function portalTemplatesRoot() {
    const fromEnv = process.env.NOTIFICATION_PORTAL_ROOT?.trim();
    if (fromEnv) {
        return fromEnv;
    }
    return (0, node_path_1.join)(SPEAKASAP_ROOT, '..', 'speakasap-portal');
}
function readTemplateBodyHtml(machineName) {
    const root = portalTemplatesRoot();
    const rel = (0, node_path_1.join)('notifications', 'templates', 'notifications', 'emails', `${machineName}.html`);
    const abs = (0, node_path_1.join)(root, rel);
    if ((0, node_fs_1.existsSync)(abs)) {
        return { body: (0, node_fs_1.readFileSync)(abs, 'utf8'), source: rel.replace(/\\/g, '/') };
    }
...
async function legacyTableExists(client, table) {
    const r = await client.query(`SELECT 1 AS ok FROM information_schema.tables
     WHERE table_schema = 'public' AND table_name = $1 LIMIT 1`, [table]);
    return r.rowCount !== null && r.rowCount > 0;
}
async function collectLegacyCounts(client) {
    const one = async (sql) => {
    ...
    if (tLetter && tTpl) {
        lettersOrphanTemplate = await one(`SELECT COUNT(*)::text AS c FROM notifications_letter l
       WHERE NOT EXISTS (SELECT 1 FROM notifications_notificationtemplate t WHERE t.id = l.template_id)`);
    }
    let templatePrefsOrphan = 0;
    if (tNs && tTpl) {
        templatePrefsOrphan = await one(`SELECT COUNT(*)::text AS c FROM notifications_notificationsettings s
...
async function verifyPostLoad(envPath) {
    loadEnvFrom(envPath);
...
async function main() {
    ...
    const envPath = (0, node_path_1.join)(SPEAKASAP_ROOT, '.env');
    ...
    const legacy = new pg_1.default.Client({ connectionString: legacyUrl, statement_timeout: 300000 });
    await legacy.connect();
    log('legacy_connected', {});
    const counts = await collectLegacyCounts(legacy);
    log('legacy_counts', counts);
    const legacyGroupIdToUuid = new Map();
    const legacyTemplateIdToUuid = new Map();
    const groupRows = await legacy
        .query(`SELECT id, machine_name, title, created FROM notifications_notificationgroup ORDER BY id`)
        .catch(() => ({ rows: [] }));
    const templateRows = await legacy
        .query(`SELECT id, machine_name, title, visible, help, settings_title
       FROM notifications_notificationtemplate ORDER BY id`)
        .catch(() => ({ rows: [] }));
    let m2mTemplateGroups = { rows: [] };
    if (await legacyTableExists(legacy, 'notifications_notificationtemplate_groups')) {
        m2mTemplateGroups = await legacy.query(`SELECT notificationtemplate_id, notificationgroup_id
       FROM notifications_notificationtemplate_groups`);
    }
    let m2mGroupManagers = { rows: [] };
    if ((await legacyTableExists(legacy, 'notifications_notificationgroup_managers')) &&
        (await legacyTableExists(legacy, 'employees_manager'))) {
        m2mGroupManagers = await legacy.query(`SELECT ngm.notificationgroup_id, em.user_id
    ...
    const letterRows = await legacy
        .query(`SELECT id, template_id, user_id, text, created, sent, recipients, from_email
       FROM notifications_letter ORDER BY id`)
        .catch(() => ({ rows: [] }));
    const inAppRows = await legacy
        .query(`SELECT id, user_id, read, text, link, created FROM notifications_notification ORDER BY id`)
        .catch(() => ({ rows: [] }));
    const ceRows = await legacy
        .query(`SELECT ces.user_id, ces.email_enabled, COALESCE(st.do_not_contact, false) AS do_not_contact
       FROM notifications_commonemailsettings ces
       LEFT JOIN students_student st ON st.user_id = ces.user_id`)
        .catch(() => {
        return legacy
            .query(`SELECT user_id, email_enabled, false AS do_not_contact FROM notifications_commonemailsettings`)
            .catch(() => ({ rows: [] }));
    });
    const prefRows = await legacy
        .query(`SELECT user_id, notification_id, active FROM notifications_notificationsettings`)
        .catch(() => ({ rows: [] }));
    ...
    if (writeDocs) {
        const docDir = (0, node_path_1.join)(SPEAKASAP_ROOT, 'docs', 'refactoring');
        const logPath = (0, node_path_1.join)(docDir, 'NOTIFICATION_DATA_MIGRATION_LOG.md');
        const block = `\n## Run ${ts()}\n\n\`\`\`json\n${JSON.stringify({
            dryRun: !doLoad,
            counts,
            skipped: {
